...
export default function CertGridClient({ certs }: { certs: Cert[] }) {
  ...
  // Focus trap and autofocus for modal
  useEffect(() => {
    if (!selected) return;
          ...
          >
            {c.image && (
              <div className="w-full h-40 mb-4 overflow-hidden rounded">
                <img src={c.image} alt={c.cert_name ?? c.issuer ?? "certificate"} className="w-full h-full object-contain" />
              </div>
            )}
                ...
                >
                  ...
                  <div className="px-6 pb-8">
                    {selected.image && (
                      <div className="w-full mb-6 flex justify-center">
                        <img
                          ...
                          className="w-full max-w-full max-h-[70vh] object-contain rounded"
                        />
                      </div>
                    )}
                    ...
                    <div className="flex gap-3 mt-4">
                      {selected.linkedin && (
                        <a href={selected.linkedin} target="_blank" rel="noreferrer" className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
                          View on LinkedIn
                        </a>
                      )}
                    </div>
